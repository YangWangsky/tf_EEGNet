...
class Model(object):
    ...
    def _build_model(self):
        with tf.name_scope('Inputs'):
            ...
            self.unbalance_cc   = tf.placeholder(tf.float32, None, name='unbalance_cc')
        ...
        with tf.name_scope('Loss'):
            self._loss = tf.losses.sparse_softmax_cross_entropy(labels=self.y_inputs, logits=self.output, weights=self.unbalance_cc)