...
# P300的数据集存在类别不平衡，标签为0比上标签为1的比例为9.237
...
decay_steps = 1*(4783//batch_size)   # 4783/batch_size为一个完整epoch所含的batch数量，1个epoch衰减一次
optimizer = tf.train.AdamOptimizer

num_epochs = 100


def unbalance_cc_change(labels):
    unbalance_cc = np.zeros(len(labels))
    labels = np.array(labels)
    id_0 = (labels==0)
    id_1 = (labels==1)
    unbalance_cc[id_0] = 1.0
    unbalance_cc[id_1] = 9.237
    return unbalance_cc
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=False):  # shuffle不选true是因为生成序号时已打乱
                ...
                summary, _, pred, loss, acc = sess.run([train_summary_op, train_op, model.output, model._loss, model._acc], {
                        model.X_inputs: inputs, model.y_inputs: targets, model.tf_is_training: True,
                        model.tf_bn_training: True, model.unbalance_cc: unbalance_cc_change(targets)})
            ...
            summary, pred, av_val_err, av_val_acc = sess.run([dev_summary_op, model.output, model._loss, model._acc], {
                    model.X_inputs: X_val, model.y_inputs: y_val, model.tf_is_training: False,
                    model.tf_bn_training: False, model.unbalance_cc: unbalance_cc_change(y_val)})
            ...
            print(sess.run(tf.argmax(pred,axis=1)))
            
            # 测试集
            summary, pred, av_test_err, av_test_acc = sess.run([test_summary_op, model.output, model._loss, model._acc], {
                    model.X_inputs: X_test, model.y_inputs: y_test, model.tf_is_training: False,
                    model.tf_bn_training: False, model.unbalance_cc: unbalance_cc_change(y_test)})
        ...
        for batch in iterate_minibatches(X_train, y_train, batch_size, shuffle=False):  # shuffle不选true是因为生成序号时已打乱
            ...
            acc = sess.run(model._acc, {
                    model.X_inputs: inputs, model.y_inputs: targets, model.tf_is_training: False,
                    model.tf_bn_training: False, model.unbalance_cc: unbalance_cc_change(targets)})
            train_acc += acc
            train_batches += 1
...
def train_all_subject(num_epochs=1):
    ...
    subj_nums, fold_pairs = Leave_Subject_Out(labels)
    EEGs_new = EEGs[0][0]
    for i in range(1,11):
        EEGs_new = np.concatenate((EEGs_new, EEGs[0][i]), axis=0)
    EEGs_new = EEGs_new.reshape(-1,8,64,1)
    labels_new = labels[0][0].reshape(-1)
    for i in range(1,11):
        labels_new = np.concatenate((labels_new, labels[0][i].reshape(-1)), axis=0)
    del EEGs, labels

    print(EEGs_new.shape)
    ...
    for subj_id in range(subj_nums):
        ...
        acc_temp = train(EEGs_new, labels_new, fold_pairs[subj_id], batch_size=batch_size, num_epochs=num_epochs, subj_id=subj_id,
